...
---@param goy_roll_manager table|nil
function M.new(
    chat,
    ace_timer,
    item,
  ...
  local function start_rolling()
    ...
    -- Candidate list is announced inside on_roll() after the server result arrives,
    -- allowing the list to be built around the rolled position.
  ...
  local function on_roll( player, roll, min, max )
    ...
    if goy_roll_manager and goy_roll_manager.is_enabled() then
      local chosen = goy_roll_manager.pick_winner( candidates, m_goy_already_won )

      if chosen then
        local ordered = goy_roll_manager.build_ordered_list( candidates, chosen, roll )
        print_players( ordered )
        actual_winner = chosen
        table.insert( m_goy_already_won, chosen.name )
      else
        -- No Chosen in raid — fall back to normal
        print_players( candidates )
        actual_winner = candidates[ roll ]
      end
    else
      print_players( candidates )
      actual_winner = candidates[ roll ]
    ...
    table.insert( m_winners, actual_winner )
